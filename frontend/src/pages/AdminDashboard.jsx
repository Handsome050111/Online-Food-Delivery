                    ...
                    return (
                        <div key={index} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <div className={`flex items-center gap-1 text-sm font-bold ${card.trend.startsWith('+') ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50'} px-2.5 py-1 rounded-lg`}>
                                    <TrendIcon size={16} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <Link to="/admin/orders" className="text-sm font-bold text-primary-600 hover:text-primary-700 bg-primary-50 px-3 py-1.5 rounded-lg transition-colors">View All</Link>
                    </div>
                                ...
                                {recentOrders.length === 0 ? (
                                    ...
                                    <tr key={index} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                                        <td className="py-4 px-2">
                                            <p className="font-extrabold text-gray-900">#{order.orderId}</p>
                                        ...
                                        <td className="py-4 px-2 font-bold text-gray-700">{order.customerName}</td>
                                        <td className="py-4 px-2 font-extrabold text-gray-900">Rs. {order.totalAmount}</td>
                                        <td className="py-4 px-2">
                                            <span className={`px-2.5 py-1 rounded-md text-xs font-bold border capitalize ${order.status === 'delivered' ? 'bg-green-50 text-green-700 border-green-200' :
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <Link to="/admin/restaurants" className="text-sm font-bold text-gray-500 hover:text-gray-900 transition-colors">Manage</Link>
                    </div>
                    <div className="space-y-4">
                        {topRestaurants.length === 0 ? (
                            <p className="text-center py-4 text-gray-500 font-bold">No top restaurants found.</p>
                        ) : topRestaurants.map((restaurant, i) => (
                            <div key={restaurant._id} className="flex items-center gap-4 group cursor-pointer p-3 -mx-3 hover:bg-gray-50 rounded-xl transition-colors">
                                <div className="w-12 h-12 bg-primary-50 rounded-xl overflow-hidden shadow-sm flex-shrink-0 flex items-center justify-center font-extrabold text-primary-600 border border-primary-100">
                                    #{i + 1}
                                </div>
                                <div className="flex-1">
                                    <h4 className="font-extrabold text-gray-900 tracking-tight group-hover:text-primary-600 transition-colors">{restaurant.name}</h4>
                                    <div className="flex items-center gap-2 mt-1">
                                        <span className="text-xs font-bold text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">{restaurant.category}</span>
