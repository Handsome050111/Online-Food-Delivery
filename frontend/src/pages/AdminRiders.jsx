    ...
    return (
        <div className="animate-in fade-in duration-500">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-8">
                <div>
                    <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">Manage Riders</h1>
                    <p className="text-gray-500 font-medium">Register and overview your delivery fleet.</p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse min-w-[800px]">
                        ...
                        <tbody className="text-sm">
                            {loading ? (
                                ...
                                riders.map((rider) => (
                                    <tr key={rider._id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                                        <td className="py-4 px-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-gray-500 font-medium">{rider.email}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="py-4 px-4">
                                        ...
                                        <td className="py-4 px-4">
                                            <span className={`px-2.5 py-1 rounded-md text-xs font-bold border capitalize ${rider.status === 'suspended' ? 'bg-red-50 text-red-700 border-red-100' : 
                                                (rider.riderDetails?.isAvailable ? 'bg-green-50 text-green-700 border-green-100' : 'bg-gray-100 text-gray-600 border-gray-200')
                                            }`}>
                                        ...
                                        <td className="py-4 px-4 text-right">
                                            <button
                                                ...
                                                className={`font-bold text-sm ${rider.status === 'suspended' ? 'text-green-600 hover:text-green-800' : 'text-red-500 hover:text-red-700'}`}
                                            >
                ...
                <div className="fixed inset-0 bg-gray-900/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-md animate-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-center p-6 border-b border-gray-100">
                            <h2 className="text-xl font-extrabold text-gray-900">Register New Rider</h2>
                        ...
                        <form onSubmit={handleAddRider} className="p-6">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input type="text" required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none" placeholder="Rider Name" />
                                </div>
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-1">Email Address</label>
                                    <input type="email" required value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none" placeholder="rider@example.com" />
                                </div>
                                <div>
                                ...
                                <div className="border-t border-gray-100 pt-4 mt-2">
                                    <h3 className="font-bold text-gray-900 mb-3 flex items-center gap-2"><Car size={16} /> Vehicle Details</h3>
                                    
                                    <div className="space-y-4">
                                        <div>
                                            <label className="block text-sm font-bold text-gray-700 mb-1">Vehicle Type</label>
                                            <select value={formData.riderDetails.vehicleType} onChange={e => setFormData({ ...formData, riderDetails: { ...formData.riderDetails, vehicleType: e.target.value } })} className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none">
                                        ...
                                        <div>
                                            <label className="block text-sm font-bold text-gray-700 mb-1">License Plate (Optional)</label>
                                            <input type="text" value={formData.riderDetails.licensePlate} onChange={e => setFormData({ ...formData, riderDetails: { ...formData.riderDetails, licensePlate: e.target.value } })} className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none" placeholder="ABC-1234" />
                                        </div>
                                    </div>
                                </div>
                            </div>
